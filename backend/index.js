...
const getOrder = require("./cartdata");
// const carttest = require("./carttest");
...
app.post("/upload", upload.single("product"), (req, res) => {
  res.json({
    success: 1,
    image_url: `https://uniform-zone.vercel.app/images/${req.file.filename}`,
  });
});
...
// Route handler to add order
app.post("/addorder", fetchuser, async (req, res) => {
  try {
    // Assuming req.user.id is set by fetchuser middleware
    let userdata = await Users.findOne({ _id: req.user.id });
    ...
    // Extract delivery details from the request body
    const { deliveryName, phoneNumber, address, postalCode } = req.body;

    // Check if mandatory fields are provided
    if (!deliveryName || !phoneNumber || !address || !postalCode) {
      return res.status(400).send("Please enter all contact details.");
    }

    // Create a new order document including delivery details
    ...
    res.status(201).send("Order created and saved successfully.");
  } catch (error) {
    console.error(error);
